...
public class UpdateTables {
	...
	public void updateStockItem() {
		...
		try {
			...
			log.flush();
			db.getCollection("stock").createIndex(new BasicDBObject("s_w_id",1).append("s_i_id", 1));
	...
	public void updateCustomer(){
		...
		try {
			...
			db.getCollection("customer").createIndex(new BasicDBObject("c_balance",-1));
	...
	public void updateOrderLine(){
		try {
			...
			log.flush();
			db.getCollection("orderline").createIndex(new BasicDBObject("ol_w_id",1).append("ol_d_id", 1).append("ol_o_id", 1));
	...
	public void updateOrder(){
		try {
			...
			log.flush();
			db.getCollection("orders").createIndex(new BasicDBObject("o_w_id",1).append("o_d_id", 1).append("o_id", 1));
